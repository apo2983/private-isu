...
func getUsersFromCache(ids []int) map[int]User {
...
func makePosts(results []Post, csrfToken string, allComments bool) ([]Post, error) {
	...
	for _, p := range results {
		...
		// users := getUsersFromCache(userIDs)
		users := getUsersFromCache(userIDs)
		for _, comment := range comments {
			if u, ok := users[comment.UserID]; ok {
				comment.User = u
			} else {
				comment.User, _ = getUser(comment.UserID)
			}
		}
...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	query += strconv.Itoa(postsPerPage)	 
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	// "u.created_at AS `user.created_at` " +
	// "FROM `posts` p JOIN `users` u ON p.user_id = u.id " + 
	// "WHERE u.del_flg=0 AND `created_at` <= ?" +
	// "ORDER BY p.created_at DESC LIMIT "
	// query += strconv.Itoa(postsPerPage)	 
...
func main() {
	...
	// dsn := fmt.Sprintf(
	// 	"%s:@unix(/var/run/mysqld/mysqld.sock)/%s?charset=utf8mb4&parseTime=true&loc=Local",
	// 	user,
	// 	password,
	// 	host,
	// 	port,
	...
	db, err = sqlx.Open("mysql", "isuconp:@unix(/var/run/mysqld/mysqld.sock)/isuconp?parseTime=true")
	if err != nil {
